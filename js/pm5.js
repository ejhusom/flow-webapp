...
var powerCanvas = document.querySelector('#powerChart');
var caloriesValues = [];
var caloriesMax = 0;
var caloriesMin = 1000;
var caloriesText = document.querySelector('#caloriesText');
var caloriesCanvas = document.querySelector('#caloriesChart');
...
class PM5 {
    ...
    _extractAdditionalStrokeData(e, multiplexed = false) {
        ...
        if (multiplexed) {
            ...
            // Visualize
            if (r.strokePower < 100) {
                powerText.innerHTML = "Do you even workout? " + r.strokePower + 'W';
            } else if (r.strokePower >= 100 && r.strokePower < 250) {
                powerText.innerHTML = "Is that all you've got? " + r.strokePower + 'W';
            } else if (r.strokePower >= 250 && r.strokePower < 400) {
                powerText.innerHTML = "Not bad. " + r.strokePower + 'W';
            } else if (r.strokePower >= 400 && r.strokePower < 580) {
                powerText.innerHTML = "Killing it! " + r.strokePower + 'W';
            } else if (r.strokePower >= 580 && r.strokePower < 720) {
                powerText.innerHTML = "BEAST MODE!!! " + r.strokePower + 'W';
            } else if (r.strokePower >= 720) {
                powerText.innerHTML = "We need to do a doping test. " + r.strokePower + 'W';
            }
            


            powerValues.push(r.strokePower);

            if (r.strokePower > powerMax) {
                powerMax = r.strokePower;
            }
            if (r.strokePower < powerMin) {
                powerMin = r.strokePower;
            }
            let powerRange = powerMax - powerMin;

            var powerPlotValues = powerValues.map(function(element) {
                return (element - powerMin)/powerRange;
            });

            if (powerValues.length > 20) {
                powerValues.shift();
            }
            
            drawWaves(powerPlotValues, powerCanvas, 1, 60);
            ...
            if (r.strokeCalories > caloriesMax) {
                caloriesMax = r.strokeCalories;
            }
            if (r.strokeCalories < caloriesMin) {
                caloriesMin = r.strokeCalories;
            }
            let caloriesRange = caloriesMax - caloriesMin;

            var caloriesPlotValues = caloriesValues.map(function(element) {
                return (element - caloriesMin)/caloriesRange;
            });

            if (caloriesValues.length > 20) {
                caloriesValues.shift();
            }
